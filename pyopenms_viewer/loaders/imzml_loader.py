...
def _drop_zero_intensity_peaks(mie) -> None:
    ...
    for spec in mie.getMSExperiment().getSpectra():
        ...
        if len(ints) == 0:
            continue
        keep = np.asarray(ints) > 0
        if keep.all():
            continue
        # OpenMS Size indices; select() also subsets Float/Integer/StringDataArrays.
        spec.select([int(i) for i in np.flatnonzero(keep)])
    ...
    def load_sync(
        ...
        try:
            ...
            spectrum_data = extract_spectrum_data(
                self.state, spectrum_stats=spectrum_stats
            )
